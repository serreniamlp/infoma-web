...
<div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        ...
        <div class="flex items-center pt-3 space-x-2 border-t border-gray-200 rounded-b">
            ...
            <form id="deleteForm" method="POST" action="{{ route('user.marketplace.seller.destroy', $product) }}" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit"
...
function deleteProduct(productId, productName) {
    ...
    document.getElementById('deleteForm').action = `/user/marketplace/sell/${productId}`;
