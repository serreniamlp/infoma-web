...
@section('title', 'Edit Hunian - EduLiving')
...
@push('styles')
    ...
    <style>
        ...
        .image-preview-item .badge-lama { position:absolute; bottom:4px; left:4px; background:#6b7280; color:#fff; font-size:10px; padding:2px 6px; border-radius:4px; font-weight:600 }
        .upload-zone { border:2px dashed #d1d5db; border-radius:12px; padding:24px; text-align:center; cursor:pointer; transition:all .2s; background:#f9fafb }
        ...
        .img-removed { opacity:.3; pointer-events:none }
        /* Tipe cards */
        .type-card { cursor:pointer; border:2px solid #e5e7eb; border-radius:12px; padding:14px; text-align:center; transition:all .2s }
        ...
        .type-icon { font-size:1.5rem; color:#9ca3af; margin-bottom:5px; transition:color .2s }
        .type-label { font-size:0.78rem; color:#6b7280; transition:color .2s }
        /* Section spesifik */
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <nav class="flex items-center gap-2 text-sm text-gray-500 mb-3">
                ...
                <i class="fas fa-chevron-right text-xs"></i>
                <a href="{{ route('provider.residence.residences.show', $residence) }}" class="hover:text-blue-600">{{ $residence->name }}</a>
                <i class="fas fa-chevron-right text-xs"></i>
                <span class="text-gray-900">Edit</span>
            </nav>
            <h1 class="text-2xl font-bold text-gray-900">Edit Hunian</h1>
            <p class="text-gray-500 mt-1 text-sm">Ubah informasi hunian <strong>{{ $residence->name }}</strong></p>
        </div>
        ...
        <form method="POST" action="{{ route('provider.residence.residences.update', $residence) }}"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                    ...
                    <p class="text-xs text-gray-500 mt-1 ml-9">Mengubah tipe akan menyesuaikan field yang tersedia</p>
                </div>
                <div class="p-6">
                    <input type="hidden" name="residence_type" id="residence_type"
                           value="{{ old('residence_type', $residence->residence_type) }}">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div class="type-card {{ $currentType === 'kontrakan' ? 'selected' : '' }}"
                        ...
                        <div class="type-card {{ $currentType === 'apartemen' ? 'selected' : '' }}"
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $residence->name) }}" required
                               class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 @error('name') border-red-400 bg-red-50 @else border-gray-300 @enderror">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4" required
                                  class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 @error('description') border-red-400 bg-red-50 @else border-gray-300 @enderror">{{ old('description', $residence->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <select name="furnish_status"
                            ...
                            <option value="unfurnished"    {{ old('furnish_status', $residence->furnish_status) === 'unfurnished'    ? 'selected' : '' }}>Unfurnished</option>
                            <option value="semi_furnished" {{ old('furnish_status', $residence->furnish_status) === 'semi_furnished' ? 'selected' : '' }}>Semi Furnished</option>
                            <option value="full_furnished" {{ old('furnish_status', $residence->furnish_status) === 'full_furnished' ? 'selected' : '' }}>Full Furnished</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            {{-- ═══════════════════════════════════════════════════ --}}
            {{-- STEP 3: DETAIL SPESIFIK PER TIPE                   --}}
            {{-- ═══════════════════════════════════════════════════ --}}

            {{-- ── KOS ──────────────────────────────────────────── --}}
            <div id="section-kos" class="type-section {{ $currentType === 'kos' ? 'active' : '' }}
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select name="kos_type"
                            ...
                            <option value="putra"  {{ old('kos_type', $residence->kos_type) === 'putra'  ? 'selected' : '' }}>Putra</option>
                            <option value="putri"  {{ old('kos_type', $residence->kos_type) === 'putri'  ? 'selected' : '' }}>Putri</option>
                            <option value="campur" {{ old('kos_type', $residence->kos_type) === 'campur' ? 'selected' : '' }}>Campur</option>
                        </select>
                        @error('kos_type')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Jumlah Kamar <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        ...
                        <input type="number" name="room_size" min="1" step="0.1" value="{{ old('room_size', $residence->room_size) }}"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
            </div>
            ...
            {{-- ── KONTRAKAN ────────────────────────────────────── --}}
            <div id="section-kontrakan" class="type-section {{ $currentType === 'kontrakan' ? 'active' : '' }}
            ...
            {{-- ── APARTEMEN ────────────────────────────────────── --}}
            <div id="section-apartemen" class="type-section {{ $currentType === 'apartemen' ? 'active' : '' }}
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select name="unit_type"
                            ...
                            <option value="studio" {{ old('unit_type', $residence->unit_type) === 'studio' ? 'selected' : '' }}>Studio</option>
                            <option value="1BR"    {{ old('unit_type', $residence->unit_type) === '1BR'    ? 'selected' : '' }}>1 Bedroom (1BR)</option>
                            <option value="2BR"    {{ old('unit_type', $residence->unit_type) === '2BR'    ? 'selected' : '' }}>2 Bedroom (2BR)</option>
                            <option value="3BR"    {{ old('unit_type', $residence->unit_type) === '3BR'    ? 'selected' : '' }}>3 Bedroom (3BR)</option>
                            <option value="4BR"    {{ old('unit_type', $residence->unit_type) === '4BR'    ? 'selected' : '' }}>4 Bedroom (4BR)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="floor_number" min="1" max="200" value="{{ old('floor_number', $residence->floor_number) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="tower_name" value="{{ old('tower_name', $residence->tower_name) }}"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"
                               placeholder="Tower A">
                    </div>
                    <div>
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <textarea name="address" id="address" rows="3" required
                                  class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 @error('address') border-red-400 bg-red-50 @else border-gray-300 @enderror">{{ old('address', $residence->address) }}</textarea>
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="price_per_month" required min="0"
                                   value="{{ old('price_per_month', $residence->price) }}"
                    ...
                    <div class="md:col-span-2 bg-blue-50 border border-blue-100 rounded-lg p-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            ...
                            <span id="slotLabel">Slot Tersedia (saat ini)</span>
                        </label>
                        <p class="text-xs text-gray-500 mb-3">
                            Berkurang otomatis saat booking disetujui. Maksimal sama dengan kapasitas
                            <strong id="capacityHint">({{ $residence->capacity }})</strong>.
                        </p>
                        <div class="flex items-center gap-4">
                            <input type="number" name="available_slots" min="0"
                                   ...
                                   class="w-36 px-4 py-2.5 border border-blue-300 bg-white rounded-lg text-sm focus:ring-2 focus:ring-blue-500 font-semibold text-blue-700">
                            <span class="text-sm text-gray-500" id="slotHint">
                                slot tersisa dari {{ $residence->capacity }} total
                            </span>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                    <h2 class="font-semibold text-gray-800 flex items-center gap-2">
                        ...
                        Foto Hunian <span class="text-xs font-normal text-gray-500 ml-1">(Maks. 10 foto total)</span>
                    </h2>
                </div>
                <div class="p-6">
                    <p id="imageError" class="hidden mb-3 text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-4 py-2">
                        <i class="fas fa-exclamation-circle mr-1"></i>Hunian harus memiliki minimal 1 foto.
                    </p>

                    @if($residence->images && count($residence->images) > 0)
                    <div class="mb-5">
                        <p class="text-sm font-medium text-gray-700 mb-3">
                            <i class="fas fa-images text-blue-500 mr-1"></i>Foto Saat Ini
                    ...
                    <div class="upload-zone" id="uploadZone" onclick="document.getElementById('images').click()">
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">Format: JPG, PNG · Maks. 5MB per file</p>
                    </div>
...
function selectType(type) {
    ...
    // Update hint label harga
    const hint = document.getElementById('slotLabel');
    if (hint) {
        hint.textContent = type === 'kos'
            ? 'Kamar Tersedia (saat ini)'
            : 'Unit Tersedia (saat ini)';
    }
}

// Init saat load — tipe sudah ada dari database
document.addEventListener('DOMContentLoaded', function () {
    const saved = document.getElementById('residence_type').value;
    if (saved) selectType(saved);
});
...
document.getElementById('hunianForm').addEventListener('submit', async function(e) {
    ...
    const totalExisting  = {{ count($residence->images ?? []) }};
    const remainingExisting = totalExisting - removedIndexes.length;
    if (remainingExisting === 0 && newFiles.length === 0) {
    ...
    // Disable section tidak aktif agar field duplikat tidak ikut terkirim
    const inactive = document.querySelectorAll('.type-section:not(.active) input, .type-section:not(.active) select');
    inactive.forEach(el => el.disabled = true);

    const formData = new FormData(this);
    formData.delete('images[]');
    newFiles.forEach(file => formData.append('images[]', file));

    inactive.forEach(el => el.disabled = false);
...
function addFiles(files) {
    files.forEach(file => {
        if (newFiles.length >= 10) { alert('Maksimal 10 foto!'); return; }
...
function removeNewFile(index) { newFiles.splice(index, 1); renderNewPreviews(); }
...
function renderNewPreviews() {
    ...
    if (newFiles.length > 0) document.getElementById('imageError').classList.add('hidden');
}

// ── FASILITAS CUSTOM TAGS ─────────────────────────────────────────────────────
