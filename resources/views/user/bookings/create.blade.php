                ...
                <form method="POST" action="{{ route('user.bookings.store') }}"
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-6">
                        ...
                        @if($type === 'residence')
                            ...
                            {{-- Info tanggal keluar otomatis --}}
                            <div class="mt-4 flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200 text-sm text-gray-600">
                                <i class="fas fa-calendar-alt text-green-500 flex-shrink-0"></i>
                                <span>Tanggal keluar:
                                    <strong id="checkout-display" class="text-gray-800">-</strong>
                                </span>
                            </div>
                        @else
                        ...
                        @if($bookable->discount_type && $bookable->discount_value)
                            <div class="flex justify-between text-green-600">
                                ...
                                <span>
                                    ...
                                    @else
                                        &minus;Rp {{ number_format($bookable->discount_value) }}/bln
                                    @endif
                                </span>
                            </div>
...
// ── Event listeners ───────────────────────────────────────────────────────────
document.getElementById('check_in_date').addEventListener('change', updateCalculation);
document.getElementById('duration_months').addEventListener('change', updateCalculation);
