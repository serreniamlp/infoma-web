                ...
                <div class="lg:col-span-2">
                                    ...
                                    <div class="absolute bottom-4 left-4 right-4">
                                        <div class="flex space-x-2 overflow-x-auto">
                                            @foreach ($residence->images as $index => $image)
                                                <img src="{{ asset('storage/' . $image) }}" alt="{{ $residence->name }}"
                                ...
                                @if ($residence->ratings_avg_rating)
                                    <div class="flex items-center">
                                        <div class="flex items-center">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <i
                                                    class="fas fa-star {{ $i <= $residence->ratings_avg_rating ? 'text-yellow-400' : 'text-gray-300' }}"></i>
                                            @endfor
                                        </div>
                                        <span class="ml-2 text-sm text-gray-600">
                                            {{ number_format($residence->ratings_avg_rating, 1) }}
                                            ({{ $residence->ratings_count }} ulasan)
                                        </span>
                                    </div>
                                @endif
                            </div>
                    ...
                    @if ($residence->latitude && $residence->longitude)
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                            ...
                            <div class="map-container">
                                <div id="residence-detail-map" style="height: 400px; width: 100%; border-radius: 8px;">
                                </div>
                                <div class="mt-4 text-sm text-gray-600">
                                    <p><strong>Alamat:</strong> {{ $residence->address }}</p>
                                    <p><strong>Koordinat:</strong> {{ number_format($residence->latitude, 6) }},
                                        {{ number_format($residence->longitude, 6) }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                                ...
                                <form id="ratingForm" class="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="review" rows="3"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ $userRating->review ?? '' }}</textarea>
                                    </div>

                                    <div class="flex items-center space-x-3">
                                        <button type="button" onclick="submitRating()"
                                            class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium">Simpan
                                            Ulasan</button>
                                        @if (isset($userRating) && $userRating)
                                            <button type="button" onclick="deleteRating({{ $residence->id }}, 'residence')"
                                                class="text-red-600 hover:text-red-700 font-medium">Hapus Ulasan</button>
                                        @endif
                                    </div>
                                </form>
                            </div>
                    ...
                    @if ($residence->ratings && $residence->ratings->count() > 0)
                        <div class="bg-white rounded-lg shadow-sm p-6">
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">Ulasan
                                ({{ $residence->ratings->count() }})</h3>
                            <div class="space-y-4">
                                        ...
                                        <div class="flex items-center justify-between mb-2">
                                            <div class="flex items-center">
                                                <div
                                                    ...
                                                    <span
                                                        class="text-sm font-medium text-blue-600">{{ substr($rating->user->name, 0, 1) }}</span>
                                                </div>
                                                <div>
                ...
                <div class="lg:col-span-1">
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-6 sticky top-8">
                        <div class="text-center mb-6">
                            ...
                            @endif
                            <div class="text-sm text-gray-600">per
                                {{ $residence->rental_period === 'monthly' ? 'bulan' : 'tahun' }}</div>
                        </div>
                    ...
                    fetch('{{ route('user.ratings.store') }}', {
                        ...
                        .then(data => {
                            if (data.status === 'success' && data.message) {
                                ...
                                // Update star display
                                highlightStars(document.querySelector(`input[name="rating"][value="${rating}"]`)
                                    .nextElementSibling, rating);
        ...
        type === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
                ...
                @if ($residence->latitude && $residence->longitude)
                    document.addEventListener('DOMContentLoaded', function() {
                        const map = L.map('residence-detail-map').setView([{{ $residence->latitude }},
                            {{ $residence->longitude }}
                        ], 15);
